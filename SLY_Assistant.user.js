	...
	*/
	...
	function waitForLabs(mutations, observer){
		...
		if(document.querySelectorAll(elemTrigger).length > 0 && !document.getElementById("assistContainer")) {
			...
			scanAutoInfoStr += '<b>auto(1):</b> optimized for Rayfams and Opods, fleets will move max 1 field (straight or diagonally)<br><b>auto(1+):</b> the same, but when the fleet wants to move to a new area, it can also move +1 sector when moving straight (if this sector yields a better chance)<br><b>auto(1,2hv):</b> optimized for Rangers, will move max 1 field diagonally or 2 fields straight (horizontally or vertically).<br><b>auto(1,2hv+):</b> the same, but when the fleet wants to move to a new area, it can also move +1 field, even if the movement lasts slightly longer than the scan cooldown<br><b>auto(1,2hv++):</b> The same, but when moving to a new area, fleets can move up to 2 fields diagonally (e.g. X+2 and Y-2)<br>';
